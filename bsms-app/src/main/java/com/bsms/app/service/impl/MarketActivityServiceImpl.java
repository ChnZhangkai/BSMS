...
public class MarketActivityServiceImpl implements MarketActivityService {
	...
	public PageInfo<TblMarketingActivity> selecTblMarketingActivities(TblMarketingActivity activity) {
		...
		for (TblMarketingActivity a : activityList) {
			...
			for (TblDicParam p : pointParams) {
				if (StringUtils.isNotBlank(a.getJfType())) {
			...
			for (TblDicParam p : cardParams) {
				if (StringUtils.isNotBlank(a.getCardType())) {
	...
	@Override
	public void addMarketingActivity(TblMarketingActivity activity) {
		
		// 新增数据默认未提交审核状态
		activity.setStatus(Constants.UNCHECK);

		tblMarketingActivityMapper.insertSelective(activity);

	}

	@Override
	public void udateMarketingActivity(TblMarketingActivity activity) {
		
		TblMarketingActivityExample example = new TblMarketingActivityExample();
		
		tblMarketingActivityMapper.updateByExampleSelective(activity, example);
